...
`versions`: a list of objects describing package versions.  Each version object can contain the
following keys:

`version`: A string containing the version of the package (format of this is up to you).

`status`: A string containing one of the following:
...
- `invalid`: This package is no longer available.

`kicad_version`: The minimum required KiCad version for this package (major.minor).

`kicad_version_max` (optional): The latest KiCad version this package is compatible with (major.minor).

`download_sha256`: A string containing a SHA-256 hash of the package archive.

`download_url`: A string containing a direct download URL for the package archive.

`download_size`: The size of the package archive, in bytes.

`install_size`: The size of the package (uncompressed), in bytes.
...
Source does require us to distribute or publish others' content.
====
