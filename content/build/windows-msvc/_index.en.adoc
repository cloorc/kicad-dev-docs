...
The variable `KICAD_RUN_FROM_BUILD_DIR` must be present (it can be set to any value).
...
Modify the value of `PYTHONPATH` and `PYTHONHOME` depending on where `vcpkg` is installed and
where your `kicad` source and build directories are located.
