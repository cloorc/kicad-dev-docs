...
<22> An optional list of <<_canonical_layer_names, canonical layer names>> which are private to 
     the footprint.
<23> An optional list of <<_net-tie_pad_groups, net tie-pad groups>>.
...
<1> The graphical items is a list of graphical <<_graphical_line,lines>>,
    ...
    <<_graphical_text_bounding_boxes,text bounding boxes>> that define the shape of the
    custom pad.  The item definitions only include the geometrical information that defines the
    item.  The text bounding box defines the location and size of the pad number and netname.
...
<3> The `layer` token defines the <<_canonical_layer_names, canonical layer>> the text resides on.  It is optionally followed by a `knockout` token indicating the text should be knocked out.
...
==== Graphical Text Bounding Box

The `gr_bbox` token defines a bounding box into which graphical text annotations (such as
pad numbers and netnames) will be shown.
