...
<4> The `at` token attributes define the X and Y coordinates and ANGLE of rotation of the
    property.
...
<1> The `pts` token defines the list of points used to draw the line(s).  There must be a
    minimum of two points.
<2> The `xy` token defines the X and Y coordinates of a point.
<3> The STROKE_DEFINITION defines how the polygon formed by the lines
    <<_stroke_definition,outline is drawn>>.
<4> The `fill` token attributes define how the polygon formed by the lines is
...
```
  (text
    "TEXT"                                                      <1>
    (at X Y ORIENTATION)                                        <2>
    (effects FONT_EFFECTS)                                      <3>
  )
```

<1> The "TEXT" attribute is a quoted string that defines the text.
<2> The `at` token defines the position coordinates and orientation angle of the text.
...
```
  (pin
    ...
    (at X Y ANGLE)                                              <3>
    (length LENGTH)                                             <4>
    (name "NAME" FONT_EFFECTS)                                  <5>
    (number "NUMBER" FONT_EFFECTS)                              <6>
  )
```
...
<3> The `at` token attributes define the X and Y coordinates and rotation ANGLE of the connection
    point of the pin relative to the symbol origin position.
<4> The `length` token attribute defines the LENGTH of the pin.
<5> The `name` token defines a quoted string containing the NAME of the pin and the FONT_EFFECTS
...
<6> The `number` token defines a quoted string containing the NUMBER of the pin and the
    FONT_EFFECTS defines how the <<_text_effects,text is displayed>>.
