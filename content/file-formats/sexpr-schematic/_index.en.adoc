...
WARNING: The fist identifier must be the root <<_hierarchical_sheet_section, sheet>> which is
...
The `bus_alias` token defines a list of net names as members of a bus in the schematic. These 
net names should be used with label attached at a bus entry or a wire attached to bus entry for 
eeschema to recognise the linkage of bus member to the bus. The bus alias section will not exist 
if there are no bus aliases in the schematic.

```
  (bus_alias
    "BUS_NAME"                                                  <1>
    (members MEMBER_LIST)                                       <2>
  )
```

<1> The `BUS_NAME` defines a unique text name for the bus.
<2> The `MEMBER_LIST` defines a list of double-quoted usable net names as members of the bus. These 
net names should be used with label attached at a bus entry or a wire attached to bus entry for 
eeschema to recognise the linkage of bus member to the bus.
