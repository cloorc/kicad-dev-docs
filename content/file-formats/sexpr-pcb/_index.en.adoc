...
The `gr_text_box` token defines a rectangle with optional border containing line-wrapped text.
...
```
  (gr_text_box
    ...
    [(start X Y)]                                               <3>
    [(end X Y)]                                                 <4>
    [(pts POLYGON_DEFINITION)]                                  <5>
    [(angle ROTATION)]                                          <6>
    (width WIDTH)                                               <7>
    (layer LAYER_DEFINITION)                                    <8>
    (tstamp UUID)                                               <9>
    (effects TEXT_STYLE)                                        <10>
...
<5> The `pts` token defines the four corners of a non-cardianlly oriented text box.
<6> The optional `angle` token defines the rotation of the text box in degrees.
<7> The `width` token defines the line-width of the border.  If -1, no border is drawn.
<8> The `layer` token defines the <<_canonical_layer_names, canonical layer>> the text box resides
    on.
<9> The `tstamp` token defines the unique identifier of the text box.
<10> The `effects` token describes the text style of the text in the text box.
...
<3> The `layer` token defines the <<_canonical_layer_names, canonical layer>> the polygon resides
    on.
